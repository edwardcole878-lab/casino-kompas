...
export function Hero({
  ...
  return (
    <section className="relative overflow-hidden gradient-hero text-primary-foreground">
      ...
      <div className="container relative mx-auto max-w-[1240px] px-6 pt-16 pb-24 md:pt-16 md:pb-[120px]">
        {eyebrow && (
        ...
        <h1 className="max-w-3xl text-[34px] font-extrabold leading-[1.15] tracking-tight md:text-[44px] lg:text-[52px]">{title}</h1>
        <p className="mt-5 max-w-2xl text-base text-white/85 md:text-lg">{subtitle}</p>
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <Button asChild size="lg" className="h-[52px] gradient-cta cta-glow px-6 text-base text-gold-foreground font-extrabold shadow-gold">
            <a href={primaryCta.to}>{primaryCta.label}</a>
          </Button>
          {secondary && (
            <Button asChild size="lg" variant="outline" className="h-[52px] border-white/30 bg-white/5 px-6 text-white hover:bg-white/15 hover:text-white">
